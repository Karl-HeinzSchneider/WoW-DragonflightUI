...
-- TEMP diagnostics (/df chardump): per-slot render state for the missing
-- slot-frame-art investigation. Dumps sizes, frame levels (slots vs the
-- pane inset - if the inset level is >= the buttons', its textures cover
-- the buttons' BACKGROUND-layer art), texture objects with path/layer/
-- shown, into the copyable hoverlag window.
function DragonflightUIMixin:DumpCharSlotDiag()
    local out = {}
    local function add(fmt, ...)
        out[#out + 1] = string.format(fmt, ...)
    end
    local function texInfo(label, tex)
        if not tex then return label .. '=nil' end
        local layer, sub = tex:GetDrawLayer()
        return string.format('%s={tex=%s layer=%s/%d shown=%s size=%.0fx%.0f}', label,
                             tostring(tex:GetTexture()), tostring(layer), sub or 0, tostring(tex:IsShown()),
                             tex:GetWidth() or 0, tex:GetHeight() or 0)
    end

    add('charslot diag %s', date('%H:%M:%S'))
    for _, fname in ipairs({'CharacterFrame', 'PaperDollFrame', 'PaperDollItemsFrame'}) do
        local f = _G[fname]
        if f then
            add('%s: level=%d strata=%s shown=%s size=%.0fx%.0f', fname, f:GetFrameLevel(), f:GetFrameStrata(),
                tostring(f:IsShown()), f:GetWidth(), f:GetHeight())
        else
            add('%s: MISSING', fname)
        end
    end
    local inset = _G['DragonflightUICharacterFrameInset']
    if inset then
        add('DFInset: level=%d strata=%s shown=%s', inset:GetFrameLevel(), inset:GetFrameStrata(),
            tostring(inset:IsShown()))
        add('  %s', texInfo('retailBg', _G['DragonflightUICharacterPanelBackground']))
    else
        add('DFInset: MISSING')
    end

    for _, slotName in ipairs({'Head', 'Hands', 'MainHand'}) do
        local btn = _G['Character' .. slotName .. 'Slot']
        if btn then
            add('%sSlot: level=%d strata=%s shown=%s size=%.0fx%.0f parent=%s styled=%s', slotName,
                btn:GetFrameLevel(), btn:GetFrameStrata(), tostring(btn:IsShown()), btn:GetWidth(), btn:GetHeight(),
                btn:GetParent() and btn:GetParent():GetName() or '?', tostring(btn.DFSlotStyled))
            add('  %s', texInfo('icon', btn.icon or _G['Character' .. slotName .. 'SlotIconTexture']))
            add('  %s', texInfo('normal', btn:GetNormalTexture()))
            add('  %s', texInfo('pushed', btn:GetPushedTexture()))
            add('  %s', texInfo('highlight', btn:GetHighlightTexture()))
            add('  %s', texInfo('IconBorder', btn.IconBorder))
            -- our created textures: enumerate regions and pick ours by texture path
            local n = 0
            for _, region in ipairs({btn:GetRegions()}) do
                if region:GetObjectType() == 'Texture' then
                    n = n + 1
                    local p = tostring(region:GetTexture())
                    if p:find('BagsItemSlot2x') or p:find('quickslot2') or p:find('bagsitemslot2x') or
                        p:find('ui%-quickslot2') then add('  ours: %s', texInfo('region' .. n, region)) end
                end
            end
            add('  regions total=%d', n)
        else
            add('%sSlot: MISSING', slotName)
        end
    end

    for _, line in ipairs(out) do print(line) end
    if DF.db and DF.db.global then DF.db.global.hoverDiagLog = out end
    if DF.HoverDiag and DF.HoverDiag.Report then DF.HoverDiag:Report() end
end
