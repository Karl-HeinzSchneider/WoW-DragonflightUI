...
local L_CN = LibStub("AceLocale-3.0"):GetLocale("DragonflightUI")
...
L["ModuleTooltipActionbar"] = "该模块全面改造默认动作条，包括微型菜单和背包按钮。\n为动作条1-8、宠物/经验/声望/控制/姿态/图腾条、背包和微型菜单提供独立选项。"
...
L["ModuleTooltipDarkmode"] = "该模块为DragonflightUI的多个框架添加暗黑模式。\n开发中 - 请反馈意见！"
L["ModuleTooltipMinimap"] = "该模块全面改造默认小地图和任务追踪器。\n为小地图和任务追踪器提供独立选项。"
L["ModuleTooltipTooltip"] = "该模块增强游戏提示框功能。\n开发中"
L["ModuleTooltipUI"] = "该模块为角色窗口等界面添加现代UI风格，并针对怀旧服重做法术书、天赋窗和专业窗口。"
L["ModuleTooltipUnitframe"] = "该模块全面改造默认单位框架，添加职业染色等功能。\n为玩家/宠物/目标/焦点/小队框架提供独立选项。"
L["ModuleTooltipUtility"] = "该模块提供通用UI功能和调整。\n开发中"

L["ModuleConditionalMessage"] = "'|cff8080ff%s|r'已停用，但相关功能已被挂钩，请执行'|cff8080ff/reload|r'重载界面！"
...
L["ConfigToolbarCoffeeTooltip1"] = "每个点赞分享都是支持，若您愿意，也可以请开发者喝杯咖啡继续开发！"
...
L["PositionTableCustomAnchorFrameDesc"] = "使用指定名称的框架作为锚点(如有效)。例如'CharacterFrame', 'TargetFrame'..."
...
L["ExtraOptionsPresetDesc"] = "将缩放、锚点、X/Y等位置相关设置设为所选预设值，但不影响其他设置。"
...
L["BuffsOptionsUseStateHandlerDesc"] = "禁用此项将导致上方可见性设置失效，但可能提升其他插件兼容性(如MinimapAlert)，因为它不会使框架变为安全框架。"
...
L["CastbarTableAutoAdjustDesc"] = "根据增益/减益数量自动调整Y轴偏移 - 当施法条锚定在目标/焦点框架下方时很有用"
...
L["MinimapUseStateHandlerDesc"] = "禁用此项将导致上方可见性设置失效，但可能提升其他插件兼容性(如MinimapAlert)，因为它不会使框架变为安全框架。"
...
L["TargetFrameFadeOutDistanceDesc"] = "设置触发淡出效果的距离(码)。\n注意: 并非所有值都有效，因使用'LibRangeCheck-3.0'库。\n计算方式为'最小距离 >= 淡出距离'"
