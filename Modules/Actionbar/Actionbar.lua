...
-- TEMP diagnostics: /df bardump appends the main-bar border geometry to
-- DragonflightUIPerfLog; /reload writes it to disk for offline analysis.
function Module.DumpBarGeometry()
    local log = DragonflightUIPerfLog
    if not log then return end
    local function add(fmt, ...) log[#log + 1] = string.format(fmt, ...) end
    local function rect(f)
        ...
        return string.format('l=%.1f b=%.1f w=%.1f h=%.1f scale=%.2f es=%.3f effRect[%.1f,%.1f -> %.1f,%.1f]', l, b, w,
                             h, s, es, l * es, b * es, (l + w) * es, (b + h) * es)
    end
    local bar = Module.bar1
    add('BARDUMP bar1: %s', rect(bar))
    if not bar then return end
    local st = bar.state
    add('BARDUMP state: hideBorder=%s rows=%s buttons=%s buttonScale=%s padding=%s orientation=%s',
        tostring(st and st.hideBorder), tostring(st and st.rows), tostring(st and st.buttons),
        tostring(st and st.buttonScale), tostring(st and st.padding), tostring(st and st.orientation))
    add('BARDUMP MainBarFrame: %s', rect(bar.MainBarFrame))
    local border = bar.BorderArt
    add('BARDUMP BorderArt: shown=%s %s', border and tostring(border:IsShown()) or 'nil', rect(border))
    if border then
        for i = 1, border:GetNumPoints() do
            local p, rel, rp, x, y = border:GetPoint(i)
            add('BARDUMP BorderArt point%d: %s -> %s(%s) x=%.1f y=%.1f', i, tostring(p),
                (rel and rel.GetName and rel:GetName()) or tostring(rel), tostring(rp), x or 0, y or 0)
        end
    end
    add('BARDUMP gryphonL: %s', rect(bar.gryphonLeft))
    add('BARDUMP gryphonR: %s', rect(bar.gryphonRight))
    for i, b in ipairs(bar.buttonTable or {}) do
        add('BARDUMP btn%02d %s shown=%s %s', i, b:GetName() or '?', tostring(b:IsShown()), rect(b))
    end
    print('|cff0070ddDFUI:|r bar geometry dumped (' .. #log .. ' lines) - /reload to write it to disk')
end
